...
The reads should be provided as a paired list (autmatically generated by fastq_dump_Bioproject_paired).
To make your own read list select all reads and select "build paired list". 

![paired_list](https://github.com/brigidar/galaxy_workflows/blob/master/dataset_pair.png)

SNPs will be predicted by Bowtie2 mapping & Freebayes variant calling. If you are using MiSeq reads you might have to lower the coverage to 10 in the Freebayes options.
...
![worflow3](https://github.com/brigidar/galaxy_workflows/blob/master/workflow3.png)
...
The workflow uses SPAdes v.3.7.1 for *de novo* assembly of reads. 
  * Reads should be imported as fastqcssanger format and grouped into a paired dataset list.
   ![fastqcssanger]https://github.com/brigidar/galaxy_workflows/blob/master/fastqcssanger.png)
...
Annotation provided by PROKKA
  * Fill in blank options with info on bacterial species to create an annotation file
  ...
  * If annotation is for publication we recommend creating genus specific annotation files to improve gene annotation
